# ==============================================================================
# r52 — Proper-score and Betfair closing-calibration comparison
# ==============================================================================
...
const R52_EVAL = BayesianFootball.Evaluation

"Convert Betfair TWA closes into the evaluator's enriched long odds contract."
function r52_betfair_closing_odds(ds)
...
function r52_curve_frame(name, source, curve)
    return DataFrame(
...
println("\n" * "="^110)
println(" EXPERIMENT 05 · PROPER SCORING AND BETFAIR CLOSING CALIBRATION")
println("="^110)

fits = Dict(name => load_fit(db, name) for name in L50_MODEL_NAMES)
...
bf_odds = r52_betfair_closing_odds(ds)
println("  Betfair rows: $(nrow(bf_odds)) across $(length(unique(bf_odds.match_id))) matches")

metrics = R52_EVAL.AbstractScoringRule[
    R52_EVAL.LogLoss(),
    R52_EVAL.CRPS(),
    R52_EVAL.PredictionScore(),
]
...
for name in L50_MODEL_NAMES
    ...
    crps = R52_EVAL.compute_metric(R52_EVAL.CRPS(), ctx)
    model_curve = calibration_curve(ctx; n_bins = 10, source = :model)
    market_curve = calibration_curve(ctx; n_bins = 10, source = :market)
    push!(curve_frames, r52_curve_frame(name, :model, model_curve))
    push!(curve_frames, r52_curve_frame(name, :betfair, market_curve))
...
mkpath(output_dir)
CSV.write(joinpath(output_dir, "r52_proper_scores.csv"), summary)
CSV.write(joinpath(output_dir, "r52_calibration_curves.csv"), curves)
