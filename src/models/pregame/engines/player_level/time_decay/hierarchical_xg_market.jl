# src/models/pregame/engines/player_level/time_decay/hierarchical_xg_market.jl

# ==========================================
# 1. THE MODEL CONFIGURATION
# ==========================================
Base.@kwdef struct DynamicMarketXGHierarchicalPlayerTimeDecayModel{
    I<:AbstractInterceptionConfig,
    P<:HierarchicalPlayerDynamicsConfig, 
...
# ==========================================
# 2. THE TURING ENGINE (Varying Slopes)
# ==========================================
@model function build_hierarchical_xg_market_player_engine(
    # --- Base Data ---
    ...
    n_seasons::Int,
    n_months::Int,
    config::DynamicMarketXGHierarchicalPlayerTimeDecayModel
)
    # ==========================================
    # 1. LOAD COMPONENTS
    # ==========================================
    ...
    # This samples our hierarchical positional components
    ...
    # --- 2. Construct Team-Specific Multipliers (Varying Slopes) ---
    # Non-centered parameterization (NCP) for stable gradients
    w_Outfield_att_team = p_dyn.w_Outfield_att_base .+ (p_dyn.team_att_raw .* p_dyn.σ_w_att)
    w_Outfield_def_team = p_dyn.w_Outfield_def_base .+ (p_dyn.team_def_raw .* p_dyn.σ_w_def)

    # --- 3. Match-Level Strength Calculation ---
    att_h = (p_dyn.w_G_att .* home_G_ratings) .+ (view(w_Outfield_att_team, home_team_indices) .* home_Outfield)
    def_h = (p_dyn.w_G_def .* home_G_ratings) .+ (view(w_Outfield_def_team, home_team_indices) .* home_Outfield)
    
    att_a = (p_dyn.w_G_att .* away_G_ratings) .+ (view(w_Outfield_att_team, away_team_indices) .* away_Outfield)
    def_a = (p_dyn.w_G_def .* away_G_ratings) .+ (view(w_Outfield_def_team, away_team_indices) .* away_Outfield)
...
# 3. THE BUILDER
# ==========================================
function Features.required_features(model::DynamicMarketXGHierarchicalPlayerTimeDecayModel)
...
function build_turing_model(config::DynamicMarketXGHierarchicalPlayerTimeDecayModel, feature_set::FeatureSet)
...
function extract_parameters(
    model::DynamicMarketXGHierarchicalPlayerTimeDecayModel, 
    ...
    # Hierarchical weights and team raw values
    ...
    for row in eachrow(df)
        ...
        # --- 2. Construct Team-Specific Multipliers (Samples x Teams) ---
        w_Outfield_att_team = p_dyn_nt.w_Outfield_att_base .+ (p_dyn_nt.team_att_raw .* p_dyn_nt.σ_w_att)
        w_Outfield_def_team = p_dyn_nt.w_Outfield_def_base .+ (p_dyn_nt.team_def_raw .* p_dyn_nt.σ_w_def)

        # --- 3. Calculate Player-Driven Strength (Vectorized across samples) ---
        # Note: Indexing into the reconstructed multiplier matrices
        γ_att_h = h_id > 0 ? w_Outfield_att_team[:, h_id] : p_dyn_nt.w_Outfield_att_base
        γ_def_h = h_id > 0 ? w_Outfield_def_team[:, h_id] : p_dyn_nt.w_Outfield_def_base
        
        γ_att_a = a_id > 0 ? w_Outfield_att_team[:, a_id] : p_dyn_nt.w_Outfield_att_base
        γ_def_a = a_id > 0 ? w_Outfield_def_team[:, a_id] : p_dyn_nt.w_Outfield_def_base

        att_h = (p_dyn_nt.w_G_att .* h_G) .+ (γ_att_h .* h_Outfield)
        def_h = (p_dyn_nt.w_G_def .* h_G) .+ (γ_def_h .* h_Outfield)

        att_a = (p_dyn_nt.w_G_att .* a_G) .+ (γ_att_a .* a_Outfield)
        def_a = (p_dyn_nt.w_G_def .* a_G) .+ (γ_def_a .* a_Outfield)
