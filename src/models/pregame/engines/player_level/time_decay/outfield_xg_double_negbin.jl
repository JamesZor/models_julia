# src/models/pregame/engines/player_level/time_decay/outfield_xg_double_negbin.jl
#
# Double NEGATIVE-BINOMIAL outfield-player engine — the over-dispersed sibling of
# DynamicDoublePoissonXGOutfieldPlayerTimeDecayModel. Same three-pillar spine
# (goals + xG + market) and the same player-rating attack/defence + kappa + home-
# advantage structure, but the GOALS pillar is RobustNegativeBinomial(r, λ) rather
# than Poisson(λ), so it can absorb the extra scoring dispersion (e.g. Ireland
# First Division 718, V/M≈1.14 — see eda/first_division_validation).
#
# The dispersion `r` comes from the shared dispersion component, so the engine
# supports the full ladder of dispersion structures by config:
#   - HomeAwayDispersion()          → scalar r_h, r_a (home/away split)        [default]
#   - GlobalDispersion()            → single global r
#   - AdvancedVolatilityDispersion()→ HIERARCHICAL r: per-team + per-month
#                                      volatility around a global base + home offset.
# Dispersion construction mirrors team_level/time_decay/goals.jl.

# ==========================================
# 1. THE MODEL CONFIGURATION
# ==========================================
Base.@kwdef struct DynamicDoubleNegBinXGOutfieldPlayerTimeDecayModel{
    ...
    M<:Features.AbstractMarketFeatureConfig
      ...
      dispersion_config::D = HomeAwayDispersion()
...
# 2. THE TURING ENGINE
# ==========================================
@model function build_double_negbin_xg_market_player_engine(
    ...
    n_seasons::Int,
    n_months::Int,
    config::DynamicDoubleNegBinXGOutfieldPlayerTimeDecayModel
)
    # ==========================================
    # 1. LOAD COMPONENTS
    # ==========================================
    ...
    # --- Dispersion construction (mirrors team_level goals.jl) ---
...
# 3. THE BUILDER
# ==========================================
function Features.required_features(model::DynamicDoubleNegBinXGOutfieldPlayerTimeDecayModel)
...
function build_turing_model(config::DynamicDoubleNegBinXGOutfieldPlayerTimeDecayModel, feature_set::FeatureSet)
    ...
    # Mask requires BOTH sides present. xG is a Gamma observation (support x>0), so
    # a *present* xG of exactly 0.0 would make logpdf(Gamma,0)=-Inf and kill the
    # whole likelihood; floor present values to a small positive ε.
    ...
    # Market pillar: trust only matches whose inverted λ_h, λ_a are present and in a
    # plausible football range (the inversion can emit a degenerate λ on thin odds).
    ...
    return build_double_negbin_xg_market_player_engine(
...
# ==========================================
# 4. THE EXTRACTOR
#    Emits r_h / r_a so prediction routes to the DoubleNegativeBinomial score path
#    (AbstractNegBinModel default in src/predictions/score_computation/negativebinomial.jl).
# ==========================================
function extract_parameters(
    model::DynamicDoubleNegBinXGOutfieldPlayerTimeDecayModel,
    ...
    for row in eachrow(df)
        ...
        m_idx = hasproperty(row, :month_idx) ? Int(row.month_idx) : month(row.match_date)
