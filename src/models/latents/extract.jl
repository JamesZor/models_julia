...
# ==============================================================================
# 0. CHAIN-SCHEMA COMPATIBILITY
# ==============================================================================

"MCMCChains-version-independent symbol membership check without type piracy."
tpl_chain_haskey(chain::Chains, symbol::Symbol) = symbol in names(chain)

"Retained for prototype API compatibility; production uses a local helper."
tpl_haskey_is_shimmed() = false
...
latent_family(::TypesInterfaces.AbstractPoissonModel) = PoissonCountFamily()
latent_family(::TypesInterfaces.AbstractNegBinModel)  = NegBinCountFamily()

latent_family(::PreGame.DynamicPxGRecombModel)      = RecombinationFamily()
latent_family(::PreGame.DynamicRecombinedGoalsModel) = RecombinationFamily()

latent_family(::PreGame.DynamicSmileDoublePoissonGoalsLeagueTimeDecayModel) = SmilePoissonFamily()
latent_family(::PreGame.DynamicSmileDoublePoissonXGOutfieldPlayerTimeDecayModel) = SmilePoissonFamily()
...
function extract_latents(::PoissonCountFamily, model, chain::Chains,
    ...
    raw = PreGame.extract_parameters(model, oos_fixtures, feature_set, chain)
...
"""
function tpl_recomb_components(model::PreGame.DynamicPxGRecombModel,
                               chain::Chains,
    ...
    inter  = PreGame.extract_interception(chain, model.interception_config, 1)
    ...
    dyn    = PreGame.extract_dynamics(chain, model.dynamics_config, "dyn", n_teams)
    w_val  = PreGame.extract_squad_wealth(chain, model.wealth_config)
    recomb = PreGame.extract_recombination(chain, model.recomb_config)
    ...
    if all(tpl_chain_haskey(chain, s) for s in κ_sym)
    ...
    if all(tpl_chain_haskey(chain, s) for s in league_sym)
