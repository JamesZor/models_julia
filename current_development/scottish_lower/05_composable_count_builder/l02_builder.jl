...
include(joinpath(@__DIR__, "l01_components.jl"))
...
struct PoissonCountModel{I, T, H, C<:Tuple, O<:PoissonObservation, G<:AbstractRateGuard} <: CB_TI.AbstractPoissonModel
    interception::I
    dynamics::T
    home_advantage::H
    covariates::C
...
struct NegBinCountModel{I, T, H, C<:Tuple, O<:NegativeBinomialObservation, G<:AbstractRateGuard} <: CB_TI.AbstractNegBinModel
    interception::I
    dynamics::T
    home_advantage::H
    covariates::C
...
add!(b::CountModelBuilder, c) = error(
    ...
    "  To add a new family, write one `add!` method — see l02_builder.jl §3.")
...
function validate(b::CountModelBuilder)
    ...
    dyn = b.dynamics
    push!(out, cb_result("dynamics exposes days_half_life",
        dyn !== nothing && hasproperty(dyn, :days_half_life),
        dyn === nothing ? "no dynamics config" :
        hasproperty(dyn, :days_half_life) ? "half-life = $(dyn.days_half_life) days" :
    ...
    push!(out, cb_result("rate guard declared", true,
    ...
    for c in b.covariates
        ...
        (p isa UnivariateDistribution) || push!(bad_prior, String(covariate_name(c)))
    end
    push!(out, cb_result("covariate priors univariate", isempty(bad_prior),
        isempty(bad_prior) ? "all scalar weights" : "not univariate: $(join(bad_prior, ", "))"))
...
"""
    build(builder) -> PoissonCountModel | NegBinCountModel
...
"""
function build(b::CountModelBuilder)
    report = validate(b)
...
_sites_home_advantage(::CB_PG.HierarchicalLeagueHomeAdvantage)  =
    [Symbol("ha.γ_base"), Symbol("ha.σ_γ"), Symbol("ha.γ_league_raw")]
...
"""
    cb_chain_columns(model, n_teams) -> Vector{String}

The EXPANDED chain column names (vector sites unrolled into `name[i]`). Kept
separate from `cb_varinfo_sites` on purpose: `VarInfo` groups them and `Chains`
expands them, and comparing either representation against the other is how a
schema error hides.
"""
function cb_chain_columns(m::ComposableCountModel, n_teams::Int)
    cols = String[]
    for s in cb_varinfo_sites(m)
        str = String(s)
        if endswith(str, "raw_a") || endswith(str, "raw_d") ||
           endswith(str, "γ_team_raw") || endswith(str, "γ_league_raw")
            append!(cols, ["$str[$i]" for i in 1:n_teams])
        elseif endswith(str, "raw_month")
            append!(cols, ["$str[$i]" for i in 1:12])
        else
            push!(cols, str)
        end
    end
...
"""
    cb_parameter_count(model, n_teams) -> Int

Number of scalar parameters. Derived, never asserted.
"""
cb_parameter_count(m::ComposableCountModel, n_teams::Int) = length(cb_chain_columns(m, n_teams))
