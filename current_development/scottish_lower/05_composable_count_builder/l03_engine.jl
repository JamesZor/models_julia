...
#   `A[idx]`, not `view(A, idx)`. See §4 and docs/tickets/T007.
#
# ==============================================================================

using Turing
# `import`, not `using`: DynamicPPL exports `subset`, which collides with the
# DataFrames `subset` that 03_poisson_distance/l00_distance_feature.jl calls
# unqualified. Everything this file needs from DynamicPPL is qualified anyway.
import DynamicPPL
using Distributions
using DataFrames
using Dates
...
include(joinpath(@__DIR__, "l02_builder.jl"))

const CB_RobustNegativeBinomial = BayesianFootball.MyDistributions.RobustNegativeBinomial
...
@model function _observe(o::NegativeBinomialObservation,
    ...
    λ_h = exp.(η_h)
    λ_a = exp.(η_a)
    return sum(logpdf.(CB_RobustNegativeBinomial.(disp.h, λ_h), yh) .* wts) +
           sum(logpdf.(CB_RobustNegativeBinomial.(disp.a, λ_a), ya) .* wts)
end
