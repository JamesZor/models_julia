...
# from the equations rather than from `l03_engine.jl`. Same discipline as
...
function cb_loglik(::NegativeBinomialObservation, p::CBParams, data, η_h, η_a)
    ...
    return sum(data.weights .* logpdf.(CB_RobustNegativeBinomial.(r, exp.(η_h)), data.yh)) +
           sum(data.weights .* logpdf.(CB_RobustNegativeBinomial.(r, exp.(η_a)), data.ya))
end
