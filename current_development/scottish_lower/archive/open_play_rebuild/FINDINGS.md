...
## Stage 8 findings (38/38 folds passed)
...
The runner persists nonempty t+1 metadata-only OOS inventory (alignment, non-overlap, IDs/count/hash), uses one registry query over fitted plus OOS IDs, and constructs an explicit rebuild boundary whose history is every observation through `t` and whose held-out target is exactly `t+1`. This translation is required because the generic splitter's cumulative `target_match_ids` do not have the held-out semantics expected by the rebuild feature contract. The native flattened global 38-fold × four-chain queue ran with 16 dynamically filled single-thread tasks. The remaining 148 chain tasks completed in 41m41s after importing the validated fold-1 chain. An initial post-processing report incorrectly marked folds 2–38 incomplete because its checkpoint shape test compared 86 total columns against 72 parameter-only names; sampling was intact. A second resume bug compared deserialized metadata structs by object identity. Commits `86c4ea1` and `c3af3df` corrected those validators, structurally recovered all 38 atomic checkpoints, and reran post-processing without resampling.

All 38 folds passed the hard and preferred convergence gates. Across folds: maximum Rhat was 1.009949 (fold 21), minimum bulk ESS 1196.07 (fold 34), minimum tail ESS 1556.32 (fold 12), zero divergences, zero depth-cap hits, maximum depth 7, and minimum chain BFMI 0.6628. Genuine next-step inference produced 710 OOS matches and 47,570 PPD rows. Six of 1,420 team sides (0.423%) used target-only population fallback. No outcome evaluation or leaderboard has yet run.
