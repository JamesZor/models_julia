# current_development/scottish_lower/distance/r05_grid_full_champion.jl
#
# RUNNER: 40-Fold MCMC Rolling Grid for Grand Champion Models
#         (Robust NegBin + Starting-XI Squad Wealth + Travel Distance + Proxy xG + RAPM)
#
...
#
# Grid Architectures:
# 1. goals_negbin_wealth_dist_hl365_hs2   (Goals NegBin + Wealth + Distance)
# 2. pxg_apm_negbin_wealth_dist_hl365_hs2 (Grand Champion: Proxy xG + Wealth + Distance + RAPM + NegBin)
...
println("\n", "="^95)
println("STARTING 40-FOLD MCMC GRID: GRAND CHAMPION WEALTH + DISTANCE MODELS (SCOTTISH LOWER)")
println("="^95)

ds = Data.load_datastore_cached(Data.ScottishLower(), max_age_hours = 10000)
save_dir = joinpath(ROOT, "data/scottish_full_champion_grid/"); mkpath(save_dir)

dyn  = PreGame.TimeDecayDynamics(days_half_life = HL)
_tag = "hl$(Int(HL))_hs$(HS)"

specs = Tuple{String, Any}[
    ("goals_negbin_wealth_dist_$(_tag)", TeamGoalsNegBinWealthDistanceModel(
        dynamics_config = dyn,
        dispersion_config = SCOTTISH_HOMEAWAY_DISPERSION,
        name = "goals_negbin_wealth_dist_$(_tag)"
    )),
    ("pxg_apm_negbin_wealth_dist_$(_tag)", TeamPxGGoalsAPMNegBinWealthDistanceModel(
        dynamics_config = dyn,
        dispersion_config = SCOTTISH_HOMEAWAY_DISPERSION,
...
println("[INFO] Running $(length(specs)) Grand Champion model specs across 40 folds (16 cores)...")
...
println("\n", "="^95)
println("✓ 40-FOLD GRAND CHAMPION GRID COMPLETE!")
println("="^95)
