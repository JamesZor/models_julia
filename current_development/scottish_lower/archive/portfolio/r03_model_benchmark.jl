# current_development/scottish_lower_portfolio/r03_model_benchmark.jl
#
# 5-WAY HEAD-TO-HEAD PORTFOLIO BENCHMARK
#
# Compares the 5 Scottish Lower models under calibrated portfolio policies:
...
println("\n", "="^95)
println("5-WAY SCOTTISH LOWER PORTFOLIO BENCHMARK")
println("="^95)
...
for (m_name, _) in all_exprs
    ...
    isfile(cache_file) || error("Cache file missing for $m_name. Run r01_build_books.jl first.")
...
for (pol_name, pol) in policies
    println("\n", "="^95)
    println("BENCHMARK RESULTS: $pol_name (Baker-McHale 800 Draws)")
    println("="^95)
...
println("\n", "="^95)
println("PORTFOLIO BENCHMARK COMPLETE")
println("="^95)
