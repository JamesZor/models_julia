...
| Layout | One folder per model + `_protocol/` for shared gate code + `docs/` for long-term memory |
...
**Phase 0 — protocol + baseline walkthrough.** `_protocol/` gate code, `01_team_poisson/`
`v01_walkthrough.jl` covering Gates 0–5. Walking these over existing `src` code doubles as
James reading the workflow end-to-end. Any gate that fails here is a real finding about `src`.
