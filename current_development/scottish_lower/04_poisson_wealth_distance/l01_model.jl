...
function tp04_model(; half_life_days=180.0, wealth_prior=truncated(Normal(0.0,.15), lower=0.0), distance_prior=truncated(Normal(0.0,.10), lower=0.0))
    SLFeaturePoissonModel(interception_config=SLFP_PG.GlobalInterception(μ=Normal(0.2,0.1)),
        ...
        wealth_feature=SLFPOOSWealthFeature(), distance_feature=ScottishDistanceFeature(metric=:log_dist_z),
        wealth_prior=wealth_prior, distance_prior=distance_prior)
end
