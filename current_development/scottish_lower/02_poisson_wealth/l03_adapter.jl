# TP02 adapter to the shared Scottish Lower verification protocol.
using Distributions
...
struct TP02Adapter{M<:SLFeaturePoissonModel} <: AbstractSLModelAdapter
    model::M
end
TP02Adapter(; kwargs...) = TP02Adapter(tp02_model(; kwargs...))
sl_model(a::TP02Adapter) = a.model
sl_model_name(::TP02Adapter) = "02_poisson_wealth"
sl_required_features(a::TP02Adapter) = SLFP_Features.required_features(a.model)
function sl_assert_model_contract(a::TP02Adapter)
    m=a.model
...
sl_build_turing_model(a::TP02Adapter, fs) = SLFP_PG.build_turing_model(a.model, fs)
sl_params_from_varinfo(a::TP02Adapter, vi) = slfp_params(a.model, vi)
sl_equation_data(a::TP02Adapter, fs) = tp02_equation_data(a.model, fs)
sl_equation_logjoint(a::TP02Adapter,p,d) = tp02_logjoint(a.model,p,d)
sl_sampled_sites(a::TP02Adapter,n) = slfp_sites(a.model,n)
sl_parameter_row(a::TP02Adapter,p) = Float64[p.μ,p.γ,p.σ_a,p.σ_d,p.w_wealth,p.w_distance,p.raw_a...,p.raw_d...]
sl_synthetic_n_teams(::TP02Adapter,p) = length(p.raw_a)
sl_synthetic_draws(a::TP02Adapter,n,k; seed=20260826) = slfp_draws(a.model,n,k; seed)
function sl_synthetic_fixtures(a::TP02Adapter, tm; n::Int=6, unmapped::Bool=false)
...
sl_reference_extract(a::TP02Adapter,p,row,fs) = slfp_reference_extract(a.model,p,row,fs)
sl_extract_parameters(a::TP02Adapter,df,fs,ch) = SLFP_PG.extract_parameters(a.model,df,fs,ch)
sl_extract_params(a::TP02Adapter,row) = BayesianFootball.Predictions.extract_params(a.model,row)
sl_compute_score_matrix(a::TP02Adapter,p; max_goals::Int=12) = BayesianFootball.Predictions.compute_score_matrix(a.model,p; max_goals)
function sl_reference_grid(::TP02Adapter,row,draw,max_goals)
    h=[pdf(Poisson(row.λ_h[draw]),i) for i in 0:max_goals-1]; aw=[pdf(Poisson(row.λ_a[draw]),i) for i in 0:max_goals-1]; h*aw'
end
function sl_marginal_cdf_bounds(::TP02Adapter,side,row,y::Int)
    λ=side === :home ? row.λ_h : row.λ_a; (y==0 ? 0.0 : mean(cdf(Poisson(x),y-1) for x in λ), mean(cdf(Poisson(x),y) for x in λ))
end
sl_marginal_logpdf(::TP02Adapter,side,row,y::Int) = log(mean(pdf(Poisson(x),y) for x in (side === :home ? row.λ_h : row.λ_a)))
function sl_capabilities(a::TP02Adapter)
...
function sl_referee_eval(a::TP02Adapter,stage::Symbol,args...)
    if stage === :config
        return [sl_result("feature pillar contract", (true == _slfp_has_wealth(a.model)) && (false == _slfp_has_distance(a.model)), "declared wealth/distance pillars match this arm")]
