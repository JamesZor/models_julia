...
| 7 | `l06_rebuild_sampling.jl` | `r06_remote_nuts_smoke.jl` — passed 4×800 retained |
| 8 | `l07_rebuild_full_experiment.jl` | `r07_remote_full_experiment.jl` — remote full inference (not yet run) |

Stage 6 passed remotely against the cached DataStore/read-only registry: manifest-exact iteration×chain extraction, stored-map boundary-held-fixture identity fallback diagnostics, explicit three-Poisson convolution, adaptive support, and ordinary Predictions inference all passed using a deterministic synthetic multi-chain `Chains` fixture. It performed no writes or MCMC. Stage 5 adds only a model-owned `Features.create_features` adapter (it preserves generic feature behavior), validates the registry fingerprint before building, and passes a concrete array-only `equation_data(fs)` tuple into a branch-free, vectorized Turing likelihood. Its runner requires `BF_DB_URL` for the existing Stage-3 registry path and performs no writes or sampling; all DynamicPPL sampled-site, fresh/compiled ReverseDiff, ForwardDiff, finite-difference, saturation, timing, and allocation gates passed remotely, with a 0.633 ms median compiled gradient. Stage 3's runner requires `BF_DB_URL`, performs no writes by default, and checks leakage, reconciliation quarantine exclusion, stable 56→1/57→2 indexing, concrete vectors, and OOS identity fallback. Stage 4's runner reuses that exact registry/FeatureSet path and performs equation parity, smooth-saturation, thinning, and ForwardDiff checks; it contains no model or sampling. Loaders contain structs, pure builders, model/extraction/recombination helpers, and no sampling. Runners will use numbered, independently sendable REPL blocks and record observed results in a future `FINDINGS.md`.

Stage 7 passed remotely with `julia --project -t16`, core pinning, BLAS=1, and four independently prior-initialized concurrent NUTS chains. The 4×800 retained posterior passed preferred convergence (max Rhat 1.00525, minimum bulk/tail ESS 1874.98/1853.59, zero divergences), real-chain extraction, score tensors, and boundary-held-fixture PPD mechanics. Its former “OOS” checks were not genuine walk-forward OOS and do not invalidate convergence. Credential-free checkpoints and diagnostics are stored under `data/scottish_open_play_rebuild/stage7_midsize_580c82d`; no full experiment, leaderboard, or production OOS cache was created.

Stage 8 completed across all 38 genuine walk-forward folds. Its remote-only runner fixes the pooled `[56,57]`, `match_biweek`, two-history-season, `stop_early=true` splitter and asserts the remotely inventoried 38 folds (19 per target season) by default. It validates nonempty t+1 metadata-only OOS per fold (season/tournament/step/no-overlap), fetches one immutable canonical registry snapshot over fitted plus true OOS IDs, constructs each rebuild boundary explicitly with all observations through `t` as fitted history and only `t+1` as held out, then uses the project's native flattened 38-fold × four-chain queue with up to 16 dynamically filled single-thread tasks. It resumes only exact-manifest valid atomic `queued_checkpoints/split_###.jls` checkpoints (and can import valid legacy combined chains), and hard-gates each fold before metadata-only OOS inference for every target fixture. Mutable progress is atomically replaced; immutable artifacts never overwrite. `STAGE8_DRY_RUN=1` inventories boundaries without sampling. It deliberately performs no outcome evaluation or leaderboard work.

See [DESIGN.md](DESIGN.md) for the complete auditable design and acceptance gates. The session handoff, artifact paths, corrections, and next steps are in [STAGE8_HANDOFF_2026-08-24.md](STAGE8_HANDOFF_2026-08-24.md).
