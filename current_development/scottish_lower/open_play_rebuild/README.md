...
Stage 8 is implemented but unrun and is the first genuine walk-forward OOS generation. Its remote-only runner fixes the pooled `[56,57]`, `match_biweek`, two-history-season, `stop_early=true` splitter and asserts the remotely inventoried 38 folds (19 per target season) by default. It validates nonempty t+1 metadata-only OOS per fold (season/tournament/step/no-overlap), fetches one canonical registry snapshot over fitted plus true OOS IDs, constructs each rebuild boundary explicitly with all observations through `t` as fitted history and only `t+1` as held out, then processes folds sequentially with four parallel prior-initialized chains, resumes only exact-manifest valid chain checkpoints, and hard-gates each fold before metadata-only OOS inference for every target fixture. Mutable progress is atomically replaced; immutable artifacts never overwrite. `STAGE8_DRY_RUN=1` inventories boundaries without sampling. It deliberately performs no outcome evaluation or leaderboard work.
