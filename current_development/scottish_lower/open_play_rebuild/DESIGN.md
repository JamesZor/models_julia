...
7. **Remote sampling smoke.** *(Implemented; pending remote execution.)* `l06_rebuild_sampling.jl` supplies project-compatible prior initialization, atomic Serialization persistence, generic DataFrame persistence round-trip checking, and diagnostics. `r06_remote_nuts_smoke.jl` requires `julia --project -t16`, BLAS=1 and core pinning; runs four single-thread queued chains concurrently (default `800+800`, `.8`, depth 10), checkpoints every completed chain, then atomically archives the exact-manifest combined chain and credential-free provenance. It blocks metadata-only OOS extraction/score/PPD promotion unless Rhat, bulk/tail ESS, divergence, and finite-lp hard gates pass. No remote run result is claimed yet.
