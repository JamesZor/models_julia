...
**Status:** Stages 1–5 are remotely validated. Stage 6 extraction/recombination is implemented with deterministic synthetic-chain validation pending local execution; no MCMC has run. V1 is deliberately small: **only the NP-NOG component has team, league, month, and home effects.**
...
6. **Extraction/recombination.** *(Implemented; local validation pending.)* `l05_rebuild_extraction_recombination.jl` requires an exact primitive parameter-section manifest, stacks iterations×chains without touching raw storage, recomputes every transform, resolves OOS identities only through the registry and stored history map, and performs audited three-component Poisson convolution. `r05_validate_oos_recombination.jl` uses a deterministic 12×2 synthetic `Chains` fixture and metadata-only OOS rows; it has no writes or sampling.
