...
8. **Full temporal OOS inference.** *(Implemented, not yet run.)* `r07_remote_full_experiment.jl` uses exactly pooled `[56,57]`, target seasons `24/25` plus `25/26`, `match_biweek`, two history seasons, and `stop_early=true`, asserting 40 configurable folds before sampling. It fetches one credential-free persisted-by-fingerprint registry snapshot for all unique boundary and true-next-step OOS IDs, runs folds sequentially and four prior-initialized chains concurrently, and resumes only valid checkpoints matching fold boundary SHA, chain ID, sample count, and primitive manifest. Each hard diagnostic failure persists diagnostics and blocks that fold's OOS while later folds continue; unexpected errors persist sanitized artifacts (optional fail-fast). Passing folds persist metadata-only all-target OOS extraction, fallback audit, generic DataFrame round-trip, adaptive-tail score validation, and `model_inference`. Progress uses replaceable atomic state while result/checkpoint artifacts are immutable. No outcomes, evaluation, or leaderboard reuse is part of this stage.
