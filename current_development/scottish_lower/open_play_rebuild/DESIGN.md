...
**Status:** Stages 1–7 are remotely validated, including a converged four-chain single-split posterior; no full temporal experiment or leaderboard has run. V1 is deliberately small: **only the NP-NOG component has team, league, month, and home effects.**
...
6. **Extraction/recombination.** *(Remotely validated at commit `6b59173`.)* `l05_rebuild_extraction_recombination.jl` requires an exact primitive parameter-section manifest, stacks iterations×chains without touching raw storage, recomputes every transform, resolves boundary-held fixture identities only through the registry and stored history map, and performs audited three-component Poisson convolution. The deterministic 12×2-chain metadata-only OOS runner passed both leagues, target-only fallback, transform/equation parity, team swaps, adaptive-tail normalization, and ordinary `model_inference`; its 30×30×24 tensor matched normalized direct Poisson to `8.67e-17`.
7. **Remote sampling smoke.** *(Passed remotely at commit `580c82d`.)* Its prior “OOS” checks were extraction/recombination mechanics on boundary-held fixtures, not genuine walk-forward OOS; this does not invalidate the convergence diagnostics.  `l06_rebuild_sampling.jl` supplies project-compatible prior initialization, resumable atomic Serialization persistence, generic DataFrame round-trip checking, and diagnostics. With `-t16`, BLAS=1, core pinning, and four concurrent single-thread chains at `800+800`, the model achieved maximum Rhat 1.00525, minimum bulk/tail ESS 1874.98/1853.59, zero divergences, depth 7 with no cap hits, and BFMI 0.764–0.870. Exact-manifest real-chain extraction and boundary-held metadata-only score/PPD mechanics passed for 3,200 draws.
8. **Full temporal OOS inference.** *(Completed: 38/38 folds passed.)* `r07_remote_full_experiment.jl` uses exactly pooled `[56,57]`, target seasons `24/25` plus `25/26`, `match_biweek`, two history seasons, and `stop_early=true`, asserting the remotely inventoried 38 folds before sampling. For each fold it translates the generic cumulative boundary into the rebuild contract: all observations through `t` are fitted history and only genuine `t+1` IDs are held out for cutoff and prediction. It fetches one credential-free persisted-by-fingerprint global registry snapshot for all fitted and true-next-step OOS IDs, runs one native flattened 38-fold × four-chain queue (up to 16 dynamically filled single-thread tasks), and resumes only valid atomic project split checkpoints matching fold metadata, boundary SHA, chain shape, and primitive manifest. Each hard diagnostic failure persists diagnostics and blocks that fold's OOS while later folds continue; unexpected errors persist sanitized artifacts (optional fail-fast). Passing folds persist metadata-only true-next-step OOS extraction, fallback audit, generic DataFrame round-trip, adaptive-tail score validation, and `model_inference`. Progress uses replaceable atomic state while result/checkpoint artifacts are immutable. No outcomes, evaluation, or leaderboard reuse is part of this stage.
