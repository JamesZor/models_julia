# TP03 adapter to the shared Scottish Lower verification protocol.
using Distributions
...
struct TP03Adapter{M<:SLFeaturePoissonModel} <: AbstractSLModelAdapter
    model::M
end
TP03Adapter(; kwargs...) = TP03Adapter(tp03_model(; kwargs...))
sl_model(a::TP03Adapter) = a.model
sl_model_name(::TP03Adapter) = "03_poisson_distance"
sl_required_features(a::TP03Adapter) = SLFP_Features.required_features(a.model)
function sl_assert_model_contract(a::TP03Adapter)
    m=a.model
...
sl_build_turing_model(a::TP03Adapter, fs) = SLFP_PG.build_turing_model(a.model, fs)
sl_params_from_varinfo(a::TP03Adapter, vi) = slfp_params(a.model, vi)
sl_equation_data(a::TP03Adapter, fs) = tp03_equation_data(a.model, fs)
sl_equation_logjoint(a::TP03Adapter,p,d) = tp03_logjoint(a.model,p,d)
sl_sampled_sites(a::TP03Adapter,n) = slfp_sites(a.model,n)
sl_parameter_row(a::TP03Adapter,p) = Float64[p.μ,p.γ,p.σ_a,p.σ_d,p.w_wealth,p.w_distance,p.raw_a...,p.raw_d...]
sl_synthetic_n_teams(::TP03Adapter,p) = length(p.raw_a)
sl_synthetic_draws(a::TP03Adapter,n,k; seed=20260826) = slfp_draws(a.model,n,k; seed)
function sl_synthetic_fixtures(a::TP03Adapter, tm; n::Int=6, unmapped::Bool=false)
...
sl_reference_extract(a::TP03Adapter,p,row,fs) = slfp_reference_extract(a.model,p,row,fs)
sl_extract_parameters(a::TP03Adapter,df,fs,ch) = SLFP_PG.extract_parameters(a.model,df,fs,ch)
sl_extract_params(a::TP03Adapter,row) = BayesianFootball.Predictions.extract_params(a.model,row)
sl_compute_score_matrix(a::TP03Adapter,p; max_goals::Int=12) = BayesianFootball.Predictions.compute_score_matrix(a.model,p; max_goals)
function sl_reference_grid(::TP03Adapter,row,draw,max_goals)
    h=[pdf(Poisson(row.λ_h[draw]),i) for i in 0:max_goals-1]; aw=[pdf(Poisson(row.λ_a[draw]),i) for i in 0:max_goals-1]; h*aw'
end
function sl_marginal_cdf_bounds(::TP03Adapter,side,row,y::Int)
    λ=side === :home ? row.λ_h : row.λ_a; (y==0 ? 0.0 : mean(cdf(Poisson(x),y-1) for x in λ), mean(cdf(Poisson(x),y) for x in λ))
end
sl_marginal_logpdf(::TP03Adapter,side,row,y::Int) = log(mean(pdf(Poisson(x),y) for x in (side === :home ? row.λ_h : row.λ_a)))
function sl_capabilities(a::TP03Adapter)
...
function sl_referee_eval(a::TP03Adapter,stage::Symbol,args...)
    if stage === :config
        return [sl_result("feature pillar contract", (false == _slfp_has_wealth(a.model)) && (true == _slfp_has_distance(a.model)), "declared wealth/distance pillars match this arm")]
