...
function tp03_model(; half_life_days=180.0, distance_prior=truncated(Normal(0.04, 0.03), lower=0.0))
    DynamicPoissonDistanceGoalsTimeDecayModel(interception_config=SLFP_PG.GlobalInterception(μ=Normal(0.2,0.1)),
        ...
        distance_feature=ScottishDistanceFeature(metric=:log_dist_z), w_dist_prior=distance_prior)
end
