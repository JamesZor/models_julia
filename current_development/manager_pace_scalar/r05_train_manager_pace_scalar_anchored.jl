# current_development/manager_pace_scalar/r05_train_manager_pace_scalar_anchored.jl
#
# RUNNER: Train the 1-Parameter Scalar Manager Pace + Team Wealth + Market Anchored Engine on Ireland 79
#
# ==============================================================================
# EXPERIMENT SETUP
# ==============================================================================
# - Model: DynamicSmileDoublePoissonXGWealthManagerPacePlayerTimeDecayModel
# - Market Anchor: ON (market_on = true, supremacy_weight = 0.4, smile_weight = 0.4)
# - Team Wealth: TeamWealthFeature (w_wealth ~ Truncated(Normal(0.105, 0.05), lower = 0.0))
# - Manager Tactical Pace: ManagerPaceFeature (pseudo_matches = 15.0, w_pace ~ Truncated(Normal(0.05, 0.03), lower = 0.0))
...
function make_anchored_manager_pace_scalar_engine()
    return DynamicSmileDoublePoissonXGWealthManagerPacePlayerTimeDecayModel(
        ...
        market_on              = true,        # Market Anchoring ON
...
println("\n", "="^90)
println("STARTING FULL TRAINING: l2_ire79_wealth_pace_sup40_sw40 (Anchored Wealth + Pace)")
println("="^90)

task = Experiments.create_experiment_task(
    ds, make_anchored_manager_pace_scalar_engine(), "l2_ire79_wealth_pace_sup40_sw40", OUT_DIR;
...
try
    ...
    if !isempty(diag)
        ...
        if !isempty(bad)
            ...
            show(stdout, MIME"text/plain"(), first(bad, min(10, nrow(bad))))
            println()
        else
            println("✓ All parameters cleanly passed convergence check (R-hat < 1.05).")
        end
    end
catch e
...
println("\n", "="^90)
println("🎉 l2_ire79_wealth_pace_sup40_sw40 TRAINING & PERSISTENCE FINISHED")
println("="^90)
