# current_development/manager_wealth/r04_route2_manager_wealth.jl
#
# RUNNER: 5-Way Route 2 Out-Of-Sample Benchmark Evaluation
#
# Comparing:
...
# 5. Hierarchical Manager + Wealth Unanchored (`l2_ire79_mgr_wealth`)
...
banner("5-WAY ROUTE 2 BENCHMARK: LOADING ENGINES")
...
println("✓ Loaded [5] Manager + Wealth (Unanchored)  : $(exp_mgr_wealth.config.name)")
...
arm_mgr_wealth    = process_arm(ds, exp_mgr_wealth,    "5. Manager + Wealth (Unanchored)")
...
disp_rows = [
    ...
    compute_supremacy_dispersion(arm_mgr_wealth)
]
...
acc_rows = [
    ...
    compute_oos_accuracy(arm_mgr_wealth)
]
...
port_rows = vcat(run_portfolio_suite(arm_noanchor),
                 ...
                 run_portfolio_suite(arm_mgr_wealth))
port_df = DataFrame(port_rows)
shw("Portfolio Simulation Results", port_df)

out_artifact_path = joinpath(OUT_DIR, "route2_manager_wealth_vs_baselines.jls")
serialize(out_artifact_path, (
    dispersion = disp_df,
    accuracy = acc_df,
    portfolio = port_df,
    arms = (noanchor = arm_noanchor, wealth = arm_wealth, anchored = arm_anchored, wealth_anchored = arm_wealth_anchor, mgr_wealth = arm_mgr_wealth)
))

banner("5-WAY ROUTE 2 BENCHMARK COMPLETE")
