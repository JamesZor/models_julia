# current_development/manager_wealth/l02_manager_wealth_engine.jl
#
# ==============================================================================
# LOADER: Manager & Team Wealth Hierarchical Turing Engine
# ==============================================================================
#
# PURPOSE:
#   Implements `DynamicSmileDoublePoissonXGWealthManagerModel` (Unanchored).
#   Co-trains:
#     1. Squad Wealth weighting (w_wealth)
#     2. Manager Tactical Pace (Hierarchical non-centered: δ_pace ~ Normal(0, σ_pace))
#     3. Manager Supremacy Quality (Hierarchical non-centered: α_mgr ~ Normal(0, σ_mgr))
#   alongside player dynamics, time decay, true xG conversion (κ), and home advantage.
#
# ==============================================================================
...
# ==============================================================================
# SECTION 1: MANAGER SUBMODEL CONFIGS & TURING SUBMODELS
# ==============================================================================

Base.@kwdef struct HierarchicalManagerPace
    σ_pace::ContinuousUnivariateDistribution = truncated(Normal(0.0, 0.15), lower=0.0)
end

@model function build_manager_pace(config::HierarchicalManagerPace, n_managers::Int)
    σ_pace ~ config.σ_pace
    pace_raw ~ filldist(Normal(0, 1), n_managers)
    return pace_raw .* σ_pace   # (n_managers-length vector)
end

Base.@kwdef struct HierarchicalManagerQuality
    σ_mgr::ContinuousUnivariateDistribution = truncated(Normal(0.0, 0.15), lower=0.0)
end

@model function build_manager_quality(config::HierarchicalManagerQuality, n_managers::Int)
    σ_mgr ~ config.σ_mgr
    mgr_raw ~ filldist(Normal(0, 1), n_managers)
    return mgr_raw .* σ_mgr    # (n_managers-length vector)
end

# Extractors
function extract_manager_pace(chain::Chains, ::HierarchicalManagerPace, n_managers::Int)
    # Checks either mgr_pace.σ_pace or σ_pace
    sig_sym = Symbol("mgr_pace.σ_pace") in keys(chain) ? Symbol("mgr_pace.σ_pace") : :σ_pace
    sigma = (sig_sym in keys(chain)) ? vec(Array(chain[sig_sym])) : zeros(Float64, size(chain, 1) * size(chain, 3))
    n_samples = length(sigma)
    pace_mat = zeros(Float64, n_samples, n_managers)
    for i in 1:n_managers
        sym1 = Symbol("mgr_pace.pace_raw[$i]")
        sym2 = Symbol("pace_raw[$i]")
        sym = (sym1 in keys(chain)) ? sym1 : ((sym2 in keys(chain)) ? sym2 : nothing)
        if sym !== nothing
            raw = vec(Array(chain[sym]))
            pace_mat[:, i] = raw .* sigma
        end
    end
    return pace_mat
end

function extract_manager_quality(chain::Chains, ::HierarchicalManagerQuality, n_managers::Int)
    sig_sym = Symbol("mgr_qual.σ_mgr") in keys(chain) ? Symbol("mgr_qual.σ_mgr") : :σ_mgr
    sigma = (sig_sym in keys(chain)) ? vec(Array(chain[sig_sym])) : zeros(Float64, size(chain, 1) * size(chain, 3))
    n_samples = length(sigma)
    qual_mat = zeros(Float64, n_samples, n_managers)
    for i in 1:n_managers
        sym1 = Symbol("mgr_qual.mgr_raw[$i]")
        sym2 = Symbol("mgr_raw[$i]")
        sym = (sym1 in keys(chain)) ? sym1 : ((sym2 in keys(chain)) ? sym2 : nothing)
        if sym !== nothing
            raw = vec(Array(chain[sym]))
            qual_mat[:, i] = raw .* sigma
        end
    end
    return qual_mat
end
...
# ==============================================================================
# SECTION 2: MODEL CONFIGURATION STRUCT
# ==============================================================================

Base.@kwdef struct DynamicSmileDoublePoissonXGWealthManagerPlayerTimeDecayModel{
    ...
    MP<:HierarchicalManagerPace,
    MQ<:HierarchicalManagerQuality,
    R<:Features.AbstractFeatureConfig,
    W<:Features.AbstractFeatureConfig,
    MGR<:Features.AbstractFeatureConfig,
    ...
    kappa_config::K
    manager_pace_config::MP          = HierarchicalManagerPace()
    manager_quality_config::MQ       = HierarchicalManagerQuality()
    player_ratings_feature::R
    wealth_feature::W                = TeamWealthFeature()
    manager_feature::MGR             = ManagerFeature()
...
const DynamicSmileDoublePoissonXGWealthManagerModel = DynamicSmileDoublePoissonXGWealthManagerPlayerTimeDecayModel


# ==============================================================================
# SECTION 3: REQUIRED FEATURES OVERLOAD
# ==============================================================================

function Features.required_features(model::DynamicSmileDoublePoissonXGWealthManagerPlayerTimeDecayModel)
...
# ==============================================================================
# SECTION 4: TURING ENGINE DEFINITION
# ==============================================================================

@model function build_double_poisson_smile_xg_wealth_manager_player_engine(
    home_team_indices::Vector{Int},
    away_team_indices::Vector{Int},
    home_manager_indices::Vector{Int},
    away_manager_indices::Vector{Int},
    ...
    n_seasons::Int,
    n_months::Int,
    config::DynamicSmileDoublePoissonXGWealthManagerPlayerTimeDecayModel
)
    # --- 1. LOAD COMPONENTS ---
    ν_xg     ~ config.ν_xg
    w_wealth ~ config.w_wealth_prior

    mgr_pace ~ to_submodel(build_manager_pace(config.manager_pace_config, n_managers))
    mgr_qual ~ to_submodel(build_manager_quality(config.manager_quality_config, n_managers))
    ...
    # Manager terms
    pace_sum = view(mgr_pace, home_manager_indices) .+ view(mgr_pace, away_manager_indices)
    mgr_diff = view(mgr_qual, home_manager_indices) .- view(mgr_qual, away_manager_indices)

    log_λ_h = clamp.(int_m .+ view(ha, home_team_indices) .+ att_h .+ def_a .+ w_shift .+ pace_sum .+ mgr_diff, -20.0, 20.0)
    log_λ_a = clamp.(int_m                                .+ att_a .+ def_h .- w_shift .+ pace_sum .- mgr_diff, -20.0, 20.0)
...
# ==============================================================================
# SECTION 5: BUILD TURING MODEL INSTANTIATION
# ==============================================================================
...
function PreGame.build_turing_model(
    ...
    n_managers = Int(data[:n_managers])
    ...
    home_mgr_ids  = Vector{Int}(data[:flat_home_manager_id])
    away_mgr_ids  = Vector{Int}(data[:flat_away_manager_id])
    ...
    return build_double_poisson_smile_xg_wealth_manager_player_engine(
        home_ids, away_ids, home_mgr_ids, away_mgr_ids, season_ids, month_idx,
        home_goals, away_goals, match_weights,
        hG, hD, hM, hF, aG, aD, aM, aF,
        home_xg, away_xg, xg_mask,
        wealth_diff, n_strikes,
        n_teams, n_managers, n_seasons, n_months, config
    )
end
...
function PreGame.extract_parameters(
    model::DynamicSmileDoublePoissonXGWealthManagerPlayerTimeDecayModel,
    ...
    match_mgr_map = get(data, :match_manager_map, Dict{Int, Tuple{Int, Int}}())
    manager_map   = get(data, :manager_map, Dict{String, Int}())
    ...
    pace_mat = extract_manager_pace(chain, model.manager_pace_config, n_managers)
    qual_mat = extract_manager_quality(chain, model.manager_quality_config, n_managers)
    ...
    # Global smile shape φ(K): [n_samples × nK] (dummy ones in unanchored mode)
    φ_mat = ones(Float64, n_samples, nK)
    ...
    for row in eachrow(df)
        ...
        # Manager Pace & Quality lookup
        mgr_pair = get(match_mgr_map, m_id, (1, 1))
        h_mgr_id, a_mgr_id = mgr_pair
        
        δ_p_h = (h_mgr_id <= size(pace_mat, 2)) ? pace_mat[:, h_mgr_id] : zeros(n_samples)
        δ_p_a = (a_mgr_id <= size(pace_mat, 2)) ? pace_mat[:, a_mgr_id] : zeros(n_samples)
        α_m_h = (h_mgr_id <= size(qual_mat, 2)) ? qual_mat[:, h_mgr_id] : zeros(n_samples)
        α_m_a = (a_mgr_id <= size(qual_mat, 2)) ? qual_mat[:, a_mgr_id] : zeros(n_samples)

        pace_sum = δ_p_h .+ δ_p_a
        mgr_diff = α_m_h .- α_m_a

        dw = get(wealth_map, m_id, 0.0)
        w_shift = w_wealth_samples .* dw

        log_λ_h = clamp.(μ_v .+ γ_h .+ att_h .+ def_a .+ w_shift .+ pace_sum .+ mgr_diff, -20.0, 20.0)
        log_λ_a = clamp.(μ_v        .+ att_a .+ def_h .- w_shift .+ pace_sum .- mgr_diff, -20.0, 20.0)
