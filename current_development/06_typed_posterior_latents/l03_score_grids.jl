...
include(joinpath(@__DIR__, "l02_extract.jl"))

const TPL_MD = BayesianFootball.MyDistributions
const TPL_D  = BayesianFootball.Data

using BayesianFootball.Data: Market1X2, MarketBTTS, MarketOverUnder, outcomes

"The score-grid truncation the whole repository uses. 12 goals a side."
...
"""
    GridWorkspace(max_goals = TPL_MAX_GOALS)

Reusable scratch for the score-grid kernels: the two marginal PMF vectors that every
family needs, allocated once and overwritten per draw.

`p_h` / `p_a` hold probabilities for the count families and LOG-probabilities for the
negative-binomial one (§3). Reusing the same buffer for both is deliberate: the two
never coexist, and a second pair of fields would be two more things to keep in sync.
"""
struct GridWorkspace
    ...
    function GridWorkspace(max_goals::Integer = TPL_MAX_GOALS)
        ...
        return new(zeros(Float64, max_goals), zeros(Float64, max_goals), Int(max_goals))
    end
end

"""
    alloc_score_grid(l, max_goals = TPL_MAX_GOALS) -> Array{Float64,3}

A destination grid sized for one fixture of `l`: `(max_goals × max_goals × n_draws)`.
Allocate one of these per worker, not per fixture.
"""
...
One fixture's smile prediction: the ordinary score grid PLUS the per-strike pricing
curve, carried together because a smile model prices Over/Under from a different
object than everything else.

| field     | shape                       | prices                          |
|-----------|-----------------------------|---------------------------------|
| `grid`    | `max_goals² × n_draws`      | 1X2, BTTS, correct score        |
| `λ_tot`   | `n_draws`                   | O/U, via `Λ(K) = λ_tot · φ(K)`  |
| `φ`       | `n_strikes × n_draws`       | O/U                             |
| `strikes` | `n_strikes`                 | which line each φ column is      |

NAMED `SmileScoreGrid`, NOT `SmileScoreMatrix`. The briefing asks for the latter, but
`Predictions.SmileScoreMatrix` already exists with a different two-field shape
(`grid::ScoreMatrix`, `Λ::Matrix`) and `l04_parity.jl` has to hold both at once to
compare them. Two distinct types with one name in one session is a silent-wrong-answer
bug, which is the exact category this prototype exists to remove.

`Λ` is NOT precomputed into a field. It is `λ_tot[k] · φ[s,k]`, one multiply, formed
where it is used — see `price_market!` §7. The legacy container materialises the whole
`nK × n_draws` matrix up front (smile_poisson.jl:81) and then reads one row of it.
"""
...
"""
    alloc_smile_buffers(l) -> (; λ_tot, φ)

The two per-fixture pricing buffers, sized from `l`. Reuse across fixtures.
"""
...
"""
    market_keys(m) -> NTuple{N, Symbol}

The outcome symbols this market prices, in the order `price_market!` writes them.
Taken from `Data.outcomes(m)`, so the keys are the ones the rest of the pipeline
joins on.
"""
...
"Number of outcome vectors `price_market!` writes for this market."
market_arity(m) = length(market_keys(m))

"""
    alloc_market_book(market, n_draws) -> NTuple{N, Vector{Float64}}

Preallocated destination for `price_market!`, in `market_keys` order. A tuple, not a
`Dict`, so the destination is concretely typed and the pricer's stores are direct.
"""
alloc_market_book(m, nd::Integer) =
    ntuple(_ -> Vector{Float64}(undef, Int(nd)), market_arity(m))
